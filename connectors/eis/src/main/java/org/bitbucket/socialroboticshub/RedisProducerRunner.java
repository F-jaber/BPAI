...
final class RedisProducerRunner extends RedisRunner {
	...
	private static final String[] microphoneTopics = { "action_audio", "dialogflow_language", "dialogflow_context",
			"dialogflow_key", "dialogflow_agent", "dialogflow_record" };
	private static final String[] robotTopics = { "action_gesture", "action_eyecolour", "action_earcolour",
			"action_headcolour", "action_idle", "action_turn", "action_wakeup", "action_rest", "action_set_breathing",
			"action_posture", "action_stiffness", "action_play_motion", "action_record_motion", "memory_add_entry",
			"memory_set_session", "memory_set_interactant_data", "memory_get_interactant_data",
	...
	private void initialiseServices(final Jedis redis) {
		...
		for (final String identifier : this.devices.get(DeviceType.MICROPHONE)) {
			...
			p1.publish("sentiment_analysis", identifier);
		...
		while (isRunning()) {
			try {
				...
				if (next instanceof CloseAction) {
					...
					final List<String> devices = new ArrayList<>(this.devices.get(type));
					if (type == DeviceType.MICROPHONE || type == DeviceType.SPEAKER) {
						devices.addAll(this.devices.get(DeviceType.BROWSER)); // HACK for audio-enabled browser
					}
					for (final String identifier : devices) {
