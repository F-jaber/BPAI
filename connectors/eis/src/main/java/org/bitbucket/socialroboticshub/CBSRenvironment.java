...
public class CBSRenvironment extends EIDefaultImpl {
	...
	private void getDevices(final Map<DeviceType, List<String>> devices) throws ManagementException {
		...
		try (final Jedis jedis = connect()) {
			final List<String> timeFetch = jedis.time();
			final Set<String> deviceIds = jedis.zrangeByScore("user:" + this.redisUser,
					(Integer.parseInt(timeFetch.get(0)) - 60), Double.POSITIVE_INFINITY);
	...
	/**
	 * Queues 'personDetected(X,Y)' as a percept to be received by the agent. This
	 * means a face has been detected at the indicated location.
	 */
	public void addDetectedPerson(final int x, final int y) {
		this.perceptQueue.add(new Percept("personDetected", new Numeral(x), new Numeral(y)));
	}
	...
	/**
	 * Queues a sentiment as a percept to be received by the agent.
	 *
	 * @param sentiment
	 */
	public void addSentiment(final String sentiment) {
		this.perceptQueue.add(new Percept("sentiment", new Identifier(sentiment)));
	}
